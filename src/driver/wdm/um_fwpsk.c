...
NTSTATUS NTAPI FwpsCalloutRegister3(
        void *deviceObject, const FWPS_CALLOUT3 *callout, UINT32 *calloutId)
{
    UNUSED(deviceObject);
    UNUSED(callout);
    UNUSED(calloutId);
...
NTSTATUS NTAPI FwpsAcquireClassifyHandle0(
        void *classifyContext, UINT32 flags, UINT64 *classifyHandle)
{
...
void NTAPI FwpsApplyModifiedLayerData0(
        UINT64 classifyHandle, PVOID modifiedLayerData, UINT32 flags)
{
    UNUSED(classifyHandle);
