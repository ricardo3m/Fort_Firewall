...
public:
    ...
    void saveIniUser();
    void saveIniUser(IniUser &iniUser, bool onlyFlags = false);

    QVariant toPatchVariant(const IniOptions &ini, bool onlyFlags, uint editedFlags) const;
    virtual bool saveVariant(const QVariant &confVar);
...
private:
    ...
    bool validateConf(const FirewallConf &conf);
